...
from src.s3_to_dynamodb.event_parser import EventParser


@dataclass
class Pipeline:
    s3_endpoint_url: Optional[str] = field(default=os.environ.get('S3_ENDPOINT_URL', None))
    dynamodb_endpoint_url: Optional[str] = field(default=os.environ.get('DYNAMODB_ENDPOINT_URL', None))
    dynamodb_table_name: Optional[str] = field(default=os.environ.get('DYNAMODB_TABLE_NAME', None))

    event_parser: EventParser = field(default_factory=lambda: EventParser())

    s3: BaseClient = field(init=False, repr=True)
    dynamodb: boto3.session.Session.resource = field(init=False, repr=True)
    ...
    def get_latest_objects_by_year_month_from_bucket(self, bucket_name: str, prefix: str) -> dict:
        """
        ...
        latest_files_by_year_month = self.get_latest_objects_by_year_month_from_list(objects=objects)
        return latest_files_by_year_month

    def create_aggregated_metrics_by_year_month(self, data: dict, bucket_name: str) -> dict:
        """
    ...
    def execute(self, event: dict):
        try:
            ...
            bucket_name = self.event_parser.get_bucket_name(event=event)
            object_key = self.event_parser.get_object_key(event=event)

            # Retrieve the content of the S3 file
            file_content = self.s3.get_object(Bucket=bucket_name, Key=object_key)
            ...
            latest_file_by_year_month = self.get_latest_objects_by_year_month_from_bucket(bucket_name=bucket_name, prefix='raw')

            # Create aggregated metrics
            aggregated_metrics_by_year_month = self.create_aggregated_metrics_by_year_month(
                data=latest_file_by_year_month, bucket_name=bucket_name
            )
